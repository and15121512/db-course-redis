...
for i, elem in enumerate(arr):
  for j, item in enumerate(elem.items()):
    r.zadd(str(i), { f'{item[0]}: {json.dumps(item[1])}': j})
end_set = time.perf_counter()

start_get = time.perf_counter()
for i, elem in enumerate(arr):
  r.zrange(str(i), 0, 2, withscores=True)
end_get = time.perf_counter()

print(f'ZSET time: {end_set - start_set:0.4f} seconds')
print(f'ZGET time: {end_get - start_get:0.4f} seconds')

print('Output example: '+str(r.zrange(str(i), 0, 2, withscores=True)))
